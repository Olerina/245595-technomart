...
var buy = document.querySelector(".item-buy");
var basket = document.querySelector(".modal-basket");
var basketClose = basket.querySelector(".modal-content-close");
buy.addEventListener("click", function(event) {
  event.preventDefault();
  basket.classList.add("modal-write-show");
});
basketClose.addEventListener("click", function(event) {
  event.preventDefault();
  basket.classList.remove("modal-write-show");
});
window.addEventListener("keydown", function(event) {
  if (event.keyCode === 27) {
    if (basket.classList.contains("modal-write-show")) {
      basket.classList.remove("modal-write-show");
    }
  }
});
